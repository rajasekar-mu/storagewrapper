...
const defaults = {
	...
	encoding: 'utf8',
	logging: false,
	expiredInterval: 20 * 60 * 1000, /* every 2 minutes */
...
LocalStorage.prototype = {
	...
	put: function (key, datumValue, options = {}) {
	...
	batchPut: function (datumValue, options = {}) {
		//console.log(datumValue);
		for (let store in datumValue) {
			//console.log(Object.keys(datumValue[store])[0],Object.values(datumValue[store])[0]);
		  this.put(Object.keys(datumValue[store])[0],Object.values(datumValue[store])[0], options);
		}
		return 1;
	},
	...
	updateItem: async function (key, datumValue, options = {}) {
		...
		if (previousDatum && isNotExpired(previousDatum)) {
			...
			return this.put(key, datumValue, options);
		}
	},
	...
	calcTTL: function (ttl) {
		...
		// only check for undefined, if null was passed in put then we probably didn't want to use the this.options.ttl
